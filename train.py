...
def main(args):
    ...
    def loss_fn(logp, target, length, mean, logv, anneal_function, step, x1, x2):
        ...
        for split in splits:
            ...
            for iteration, batch in enumerate(data_loader):
                ...
                recon_loss, KL_loss, KL_weight = loss_fn(logp, batch['target'],
                    batch['length'], mean, logv, args.anneal_function, step, args.x1, args.x2)
            ...
            logger.info("%s Epoch %02d/%i, Mean Negative ELBO %9.4f"%(split.upper(), epoch, args.epochs, mean_loss))
            # print(mean_loss)

            if args.tensorboard_logging:
                writer.add_scalar("%s-Epoch/NegELBO"%split.upper(), mean_loss, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-x1', '--x1', type = int, default = 250)
    parser.add_argument('-x2', '--x2', type = float, default = 0.025)
